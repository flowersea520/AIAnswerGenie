...
const onAiGenerateSuccess = (result: API.QuestionContentDTO[]) => {
  ...
  // 如果是在数组前面... 用这个展开运算符的话，那么就是将数组的元素拷贝出来
  // 这里拷贝出来后，放入一个新数组中，且添加了一些 新元素
  questionContent.value = [...questionContent.value, ...result];
};

/**
 * AI 生成题目成功后执行（SSE一道一道返回）
 *  sse对象在子组件（创建了一个新的 SSE（Server-Sent Events）对象，用于从服务器接收实时更新。）
 */
const onAiGenerateSuccessSSE = (result: API.QuestionContentDTO) => {
  message.success(`AI 生成题目成功`);
  // 如果是在数组前面... 用这个展开运算符的话，那么就是将数组的元素拷贝出来
  // 这里拷贝出来后，放入一个新数组中，且添加了一个新元素（这个新元素就是后端返回的一道新题目）
  questionContent.value = [...questionContent.value, result];
};
/**
 *  sse 开始生成
 *  sse 建立连接（参数是 sse的事件对象)
 * @param event
 */
const onSSEStart = (event: any) => {
  message.success(`AI 生成题目开始`);
};
/**
 *  SSE生成题目结束
 * @param event
