...
-- 创建库
create database if not exists AIAnswerGenie;
...
) comment '用户' collate = utf8mb4_unicode_ci;   -- 应用表
...
) comment '应用' collate = utf8mb4_unicode_ci;-- 题目表
...
(
    ...
    isDelete         tinyint  default 0                 not null comment '是否删除',
    index idx_appId (appId)
) comment '评分结果' collate = utf8mb4_unicode_ci;-- 用户答题记录表
...
) comment '用户答题记录0（appId为偶数的用户答题记录表）' collate = utf8mb4_unicode_ci;
...
(
    ...
    index idx_appId (appId),
    index idx_userId (userId)
) comment '用户答题记录1（appId为奇数的用户答题记录表）' collate = utf8mb4_unicode_ci;